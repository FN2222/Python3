...
**新开一个 Cursor 会话时**,不要把 `60` 那份文件从头到尾全贴。
只复制「第一节」那个代码框。旧聊天、`00-system` 全文、方案文档都不必贴 ——
仓库打开后 `AGENTS.md` 会自动注入,其余文件它自己读。
面试复习那段等章节全部通过后再单独发。
开工前先在本机跑一次 `nlnotes next --count 1`:打不出 id 说明任务包还没生成,
这时开写作会话没有意义,先做本文第 2 节的准备阶段。
同一会话接着写下一批,改贴短的 `prompts/61-继续下一批.md`,不要再贴整份 60。
